...
if __name__ == "__main__":
    ...
    if plot_type == "vr_mollweide":        
        ...
        # UPDATED - include overdensity
        if overdensity == True:
            figname = "{}_rho_{}_faceon_{:03d}_overdensity.png".format(sim, ptype, k)
        else: 
            figname = "{}_rho_{}_faceon_{:03d}.png".format(sim, ptype, k)
        # UPDATED - include overdensity, rmin-rmax, bmin-bmax
        if overdensity == True:
            fig_title = "{} satellite rho {}; {}-{} kpc; t={:.2f}  Gyr; overdensity".format(sim, ptype, rmin, rmax, times[k])
        else:
            fig_title = "{} satellite rho {}; {}-{} kpc; t={:.2f}  Gyr".format(sim, ptype, rmin, rmax, times[k])
        pos_galactic = kinematics1.pos_cartesian_to_galactic()
...
#         print(lsat[:k-300+1]) # Should be 85 elements, should not be 0 (do the same for bsat[:k-300+1])
        ...
        figname = "outer_{}_OP_{}_faceon_no_sat_{:03d}.png".format(sim, ptype, k)
